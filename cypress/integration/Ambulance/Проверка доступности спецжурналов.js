...
describe('Спецжурналы, доступность через комбобокс', function () {
    beforeEach(function () {
        ...
        cy.getCookie('JSESSIONID').then(cook => {
            ...
            cy.wait(300);
            cy.visit('http://' + site + '/ambulance/#/journals')
        })
    })
    beforeEach(function () {
    ...
    it('Журнал вызовов с длительным временем обслуживания', function () {
        cy.visit('http://' + site + '/ambulance/#/journals/longtime')
    ...
    it('Оценка карт вызова', function () {
        cy.visit('http://' + site + '/ambulance/#/journals/evaluationCriteriaJournal')
